...
public static class SkeletonEsp
{
    ...
    public static void Draw(ImDrawListPtr drawList, GameData gameData)
    {
        ...
        var config = CS2Cheat.Utils.ConfigManager.Load();
        ...
        foreach (var entity in gameData.Entities)
        {
            ...
            {
                var hp = entity.Health / 100f;
                color = OverlayRenderer.ToColor((byte)(255 * (1 - hp)), (byte)(255 * hp), 0);
            }
            else
            {
                var colorArr = entity.Team == CS2Cheat.Core.Data.Team.Terrorists ? config.EspBoxColor : config.EspBoxColorTeam;
                color = OverlayRenderer.ToColor(new Vector4(colorArr[0], colorArr[1], colorArr[2], colorArr[3]));
            }
    ...
    private static uint GetTeamColor(Team team)
    {
        return team switch
        {
            Team.Terrorists => OverlayRenderer.Colors.Red,
            Team.CounterTerrorists => OverlayRenderer.Colors.Blue,
            _ => OverlayRenderer.Colors.White
        };
    }
