...
//    private int _lastTargetId = -1; // CS0414 - unused
    ...
    {
        if (!Config.AimRcs || GameData?.Player == null || !Config.AimBotKey.IsKeyDown())
        {
            _previousPunch = Vector2.Zero;
    ...
    private bool GetAimTargetWithPrediction(out Vector2 aimAngles, double customFov)
    {
        ...
        if (GameData.Entities == null) return false;
        foreach (var entity in GameData.Entities)
        {
            ...
            var effectiveFov = Config.AimDynamicFov
                ? customFov / Math.Max(1.0, Vector3.Distance(GameData.Player!.Origin, entity.Origin) / 100.0)
                : customFov;
            if (angleSize >= effectiveFov) continue;
