...
public abstract class EntityBase
{
    ...
    public IntPtr ControllerAddress => ControllerBase;
    ...
    protected IntPtr CurrentWeapon { get; set; }
    ...
    public virtual bool Update(GameProcess gameProcess)
    {
        ...
        if (ControllerBase == IntPtr.Zero) return false;
        ...
        {
            LifeState = gameProcess.Process.Read<byte>(AddressBase + Offsets.m_lifeState) == 0;
            ...
            HasDefuser = gameProcess.Process.Read<bool>(AddressBase + Offsets.m_bHasDefuser);
            IsDefusing = gameProcess.Process.Read<bool>(AddressBase + Offsets.m_bIsDefusing);
        }
        else
        {
            Team = gameProcess.Process.Read<int>(ControllerBase + Offsets.m_iTeamNum).ToTeam();
        }

        Ping = gameProcess.Process.Read<int>(ControllerBase + Offsets.m_iPing);

        var moneyServices = gameProcess.Process.Read<IntPtr>(ControllerBase + Offsets.m_pInGameMoneyServices);
        Money = moneyServices != IntPtr.Zero
            ? gameProcess.Process.Read<int>(moneyServices + Offsets.m_iAccount)
            : 0;

        if (CurrentWeapon != IntPtr.Zero)
        {
            Ammo = gameProcess.Process.Read<int>(CurrentWeapon + Offsets.m_iClip1);
            IsReloading = gameProcess.Process.Read<bool>(CurrentWeapon + Offsets.m_bInReload);
        }
    ...
    private IntPtr ReadCurrentWeapon(GameProcess gameProcess)
    {
        ...
        if (weaponHandle <= 0) return IntPtr.Zero;
